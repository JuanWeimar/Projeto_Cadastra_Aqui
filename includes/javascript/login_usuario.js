window.onload = function(){
    login_usuario.addEventListener("submit", validaFormulario)
    login_empresa.addEventListener("submit", validaFormularioEmpresa)
    //login_usuario.addEventListener("click", function(){vazia(this,event)})
    login_empresa.cnpj.addEventListener("keypress", function(){numeros(event,event.keyCode)})
    login_empresa.cnpj.addEventListener("keypress", function(){mascara_cnpj(this,event)})
    login_usuario.cpf.addEventListener("keypress", function(){numeros(event,event.keyCode)})
    login_usuario.cpf.addEventListener("keypress", function(){mascara_cpf(this,event)})
}
    function validaFormulario(event)
    {
        event.preventDefault()
            var campo_cpf = document.querySelectorAll('.input-cpf'),
                campo_senha_cand = document.querySelectorAll('.input-senha')
                submeter_cand=true;
    
            var cpf = document.getElementById('cpf').value.replace(/[^\d]+/g,'')
        if(TestaCPF(cpf)==false)
        {
            document.getElementById('cpf').classList.add('invalid-usuario')
            document.getElementById('mensagem').innerHTML="CPF Inválido"
            submeter_cand=false
        }
    

    campo_cpf.forEach(function (campo, indice) {
        
        if(campo.value=="")
        {
            
            campo.classList.add('invalid-usuario')
            
            document.getElementById('mensagem').innerHTML="Campos Obrigatorios"
            submeter_cand=false
            campo.addEventListener("change", function(){campo.classList.remove('invalid-usuario')});
        }
        else
        {
            if(campo.classList.contains('invalid-usuario')==true)
            {
                campo.classList.remove('invalid-usuario')
            }
    ...
    campo_senha_cand.forEach(function (campo, indice) {
        ...
        if(submeter_cand==true)
        {
            document.getElementById("login_usuario").submit();
        }
    }

    function validaFormularioEmpresa(event)
    {
        event.preventDefault()
            var campo_cnpj = document.querySelectorAll('.input-cnpj'),
                campo_senha = document.querySelectorAll('.input-senha-empresa')
                submeter=true;
    
            var cnpj = document.getElementById('cnpj').value.replace(/[^\d]+/g,'')
        if(TestaCNPJ(cnpj)==false)
        {
            document.getElementById('cnpj').classList.add('invalid-empresa')
            document.getElementById('mensagem-empresa').innerHTML="CNPJ Inválido"
            submeter=false
        }
    

    campo_cnpj.forEach(function (campo, indice) {
        
        if(campo.value=="")
        {
            
            campo.classList.add('invalid-empresa')
            
            document.getElementById('mensagem-empresa').innerHTML="Campos Obrigatorios"
            submeter=false
            campo.addEventListener("change", function(){campo.classList.remove('invalid-empresa')});
        }
        else
        {
            if(campo.classList.contains('invalid-empresa')==true)
            {
                campo.classList.remove('invalid-empresa')
            }
        ...
        campo_senha.forEach(function (campo, indice) {
        ...
        if(submeter==true)
        {
            document.getElementById("login_empresa").submit();
        }
    }
